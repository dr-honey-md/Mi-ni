document.addEventListener("DOMContentLoaded", () => {
  ...
  const categoryIcons = {
    admin: 'fa-user-shield', fun: 'fa-face-laugh', downloader: 'fa-download',
    utility: 'fa-toolbox', other: 'fa-shapes', group: 'fa-users',
    owner: 'fa-crown', converter: 'fa-arrows-rotate', search: 'fa-magnifying-glass'
  };
    ...
    sortedCats.forEach(cat => {
      ...
      html += `<div class="commands-category-group">
        <div class="commands-category-title">
          <i class="fas ${icon}"></i> ${cat} <span class="count-chip">${groups[cat].length}</span>
        </div>`;
  ...
  (function() {
    ...
    function tick() {
      ...
      uptimeText.textContent = fmtUptime(baseUptimeSec + elapsed);
    }

    function syncUptime() {
      fetch('/api/uptime').then(r => r.json()).then(d => {
        baseUptimeSec = d.uptimeSec || 0;
        lastSync = Date.now();
        tick();
      }).catch(() => {});
    }
    ...
    setInterval(syncUptime, 60000); // resync every minute to stay accurate
  })();
  ...
  (function() {
    ...
    const translations = {
      en: {
        ...
        referral_title: "Invite Friends, Climb the Leaderboard",
        view_leaderboard: "View Leaderboard",
        faq_title: "FAQ & Help",
        leaderboard_title: "Top Inviters Leaderboard",
        tab_daily: "Daily", tab_weekly: "Weekly", tab_all: "All-Time"
      },
      ur: {
        ...
        referral_title: "دوستوں کو مدعو کریں، لیڈربورڈ میں آگے بڑھیں",
        view_leaderboard: "لیڈربورڈ دیکھیں",
        faq_title: "سوالات و مدد",
        leaderboard_title: "ٹاپ مدعو کنندگان لیڈربورڈ",
        tab_daily: "روزانہ", tab_weekly: "ہفتہ وار", tab_all: "تمام وقت"
      },
      hi: {
        ...
        referral_title: "दोस्तों को बुलाएं, लीडरबोर्ड में आगे बढ़ें",
        view_leaderboard: "लीडरबोर्ड देखें",
        faq_title: "सहायता और सवाल",
        leaderboard_title: "टॉप इनवाइटर्स लीडरबोर्ड",
        tab_daily: "दैनिक", tab_weekly: "साप्ताहिक", tab_all: "हर समय"
      },
      ar: {
        ...
        referral_title: "ادعُ أصدقاءك وتصدّر لوحة المتصدرين",
        view_leaderboard: "عرض لوحة المتصدرين",
        faq_title: "الأسئلة الشائعة والمساعدة",
        leaderboard_title: "لوحة متصدري الدعوات",
        tab_daily: "يومي", tab_weekly: "أسبوعي", tab_all: "كل الأوقات"
      }
    };
  ...
  (function() {
    ...
    const faqs = [
      ...
      { q: "How does the referral system work?", a: "Every visitor gets a unique invite link. When someone pairs using your link, you get credit on the leaderboard. Share your link from the referral section above the phone input." },
      { q: "How is the leaderboard ranked?", a: "The leaderboard ranks users by how many successful new pairings came from their referral link, shown for Daily, Weekly, and All-Time periods." },
  ...
  (function() {
    ...
    function getOwnerId() {
      let id = localStorage.getItem('drHoneyDeviceId');
      if (!id) {
        id = 'dev_' + Math.random().toString(36).slice(2, 10) + Date.now().toString(36);
        localStorage.setItem('drHoneyDeviceId', id);
      }
      return id;
    }
    ...
    fetch('/api/referral/create', {
      ...
      if (d.code) {
        myReferralLink = buildLink(d.code);
        linkInput.value = myReferralLink;
      }
    }).catch(() => {
      linkInput.value = 'Could not generate link. Try refreshing.';
    });
    ...
    if (shareBtn) {
      shareBtn.addEventListener('click', () => {
        if (!myReferralLink) return;
        if (navigator.share) {
          navigator.share({ title: 'DR-HONEY-MINI', text: 'Pair your WhatsApp with this bot!', url: myReferralLink }).catch(() => {});
        } else {
          navigator.clipboard.writeText(myReferralLink).then(() => showToast('Link copied to clipboard!', 'success')).catch(() => {});
        }
      });
    }
  })();
  ...
  (function() {
    ...
    function loadLeaderboard(period) {
      listEl.innerHTML = '<div class="leaderboard-loading"><i class="fas fa-spinner fa-spin"></i> Loading...</div>';
      fetch(`/api/referral/leaderboard?period=${period}`)
        .then(r => r.json())
        .then(d => {
          const list = d.leaderboard || [];
          if (!list.length) {
            listEl.innerHTML = '<div class="leaderboard-empty"><i class="fas fa-ghost"></i><br>No invites yet. Be the first!</div>';
            return;
          }
          listEl.innerHTML = list.map((item, i) => `
            <div class="leaderboard-row">
              <div class="leaderboard-rank">${i + 1}</div>
              <div class="leaderboard-code">${item.code}</div>
              <div class="leaderboard-count">${item.count} invite${item.count === 1 ? '' : 's'}</div>
            </div>
          `).join('');
        })
        .catch(() => {
          listEl.innerHTML = '<div class="leaderboard-empty"><i class="fas fa-triangle-exclamation"></i><br>Could not load leaderboard.</div>';
        });
    }
