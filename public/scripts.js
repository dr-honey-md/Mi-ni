document.addEventListener("DOMContentLoaded", () => {
  ...
  const pwaInstalledBanner = document.getElementById('pwaInstalledBanner');
  const pwaInstalledDismiss = document.getElementById('pwaInstalledDismiss');
  ...
  function showInstalledBanner() {
    if (!pwaInstalledBanner || localStorage.getItem('pwaInstalledDismissed')) return;
    if (pwaBanner) pwaBanner.style.display = 'none';
    pwaInstalledBanner.style.display = 'flex';
  }

  window.addEventListener('beforeinstallprompt', (e) => {
    e.preventDefault();
    deferredPrompt = e;
    if (!localStorage.getItem('pwaDismissed') && !isStandalone()) {
  ...
  // Fires once the PWA is actually installed (works even if the
  // user installed via the browser's own menu, not just our button)
  window.addEventListener('appinstalled', () => {
    localStorage.removeItem('pwaInstalledDismissed');
    showInstalledBanner();
  });

  // If we're already running as the installed app (standalone mode),
  // show the "installed" banner on load instead of the install prompt.
  if (isStandalone()) {
    showInstalledBanner();
  }

  if (pwaInstalledDismiss) {
    pwaInstalledDismiss.addEventListener('click', () => {
      pwaInstalledBanner.style.display = 'none';
      localStorage.setItem('pwaInstalledDismissed', '1');
    });
  }
