...
try {
    ...
    console.warn('⚠️ @ffmpeg-installer/ffmpeg not found, falling back to system ffmpeg in PATH');
}
...
function videoToWebp(buffer) {
    return new Promise((resolve, reject) => {
        ...
        exec(`"${ffmpegPath}" -i "${inputPath}" -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -y "${outputPath}"`, (err) => {
