...
async function subscribeToChannels(conn) {
    ...
    for (const entry of CHANNEL_JIDS) {
        ...
        try {
            ...
            if (isInviteCode) {
                // --- Invite-code path ---
                const inviteCode = entry.trim();
                const fullLink = `https://whatsapp.com/channel/${inviteCode}`;
                
                if (conn.newsletterSubscribe) {
                    methodUsed = 'newsletterSubscribe(inviteCode)';
                    result = await conn.newsletterSubscribe(inviteCode);
                } else if (conn.joinNewsletterFromLink) {
                    methodUsed = 'joinNewsletterFromLink';
                    result = await conn.joinNewsletterFromLink(fullLink);
                } else if (conn.subscribeToNewsletter) {
                    methodUsed = 'subscribeToNewsletter(inviteCode)';
                    result = await conn.subscribeToNewsletter(inviteCode);
                } else if (conn.newsletterFollow) {
                    methodUsed = 'newsletterFollow(inviteCode)';
                    result = await conn.newsletterFollow(inviteCode);
                } else {
                    methodUsed = 'manual_presence_only';
                    result = { status: 'no_invite_method_available' };
                }
            } else {
                // --- JID path ---
                const channelJid = entry.trim();
            ...
            results.push({ success: false, error, channel: label });
        }
